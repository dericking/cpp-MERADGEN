  ...
  :rndmFlag("off")
{
  ...
  fTargPol = 0.;
  fLEcorFac = 1.;
  // RC model: 0 none, 1 Alexander/structure-function, 2 MERADGEN Approach A.
  SetRCtype(0);
...
void MolPolPrimaryGeneratorAction::SetRCtype(G4int t)
{
  if (t < 0 || t > 2) {
    G4Exception("MolPolPrimaryGeneratorAction::SetRCtype", "BadRCtype",
                JustWarning, "RCtype must be 0, 1, or 2; leaving previous value.");
    return;
  }
  fRCtype = t;
}
...
void MolPolPrimaryGeneratorAction::GeneratePrimaries(G4Event* anEvent)
{
 ...
 fDefaultEvent->Reset();
  ...
  if( gentype == "moller" )
    {
      MollerVertex v;
      SampleMollerVertex(v);

      // Radiative model: one readable branch per RCtype (like gentype == "beam").
      if( fRCtype == 0 ){
        GenerateMollerBorn(anEvent, v);
      } else if( fRCtype == 1 ){
        GenerateMollerAlexander(anEvent, v);
      } else if( fRCtype == 2 ){
        GenerateMollerMeradgen(anEvent, v);
      } else {
        G4Exception("MolPolPrimaryGeneratorAction::GeneratePrimaries", "BadRCtype",
                    JustWarning, "Unknown RCtype; using Born (RCtype=0).");
        GenerateMollerBorn(anEvent, v);
      }
    }
...
void MolPolPrimaryGeneratorAction::SampleMollerVertex(MollerVertex& v)
{
v.beamE = fBeamE;
      ...
      v.thcom = acos(G4RandFlat::shoot(cos(fthetaComMax), cos(fthetaComMin)));
      v.phcom = G4RandFlat::shoot(fphiMin, fphiMax); //deg

      //v.zpos: random position within the target of length fTargLen (local coordinate)
      // vertex z position is calculated as vtx_z = v.zpos + fZ (target center) later
      v.xpos = G4RandGauss::shoot( fX, fXsmear );
      v.ypos = G4RandGauss::shoot( fY, fYsmear );
      v.zpos = ( G4UniformRand() - 0.5 ) * fTargLen;
      ...
      msThick[0] = ((v.zpos + fTargLen/2) * ironDensity ); // in (g/cm2);
      ...
      v.direction = G4ThreeVector(0,0,1.);
      ...
      v.direction.rotateX(-fBeamRotZY);
      v.direction.rotateY( fBeamRotZX);

      //~~apply MS
      v.direction.rotateY(msth);
      v.direction.rotateX(msph);
      ...
      if (prob_sample <= prob) {
        ...
        v.beamE = fBeamE - eloss;
        assert( v.beamE > electron_mass_c2 );
      } else {
        v.beamE = fBeamE;
      }
      ...
      v.zLum = msZ[0] * ironDensity * (cm3/g) * (v.zpos + fTargLen/2) / cm * Avogadro / msA[0];
}

void MolPolPrimaryGeneratorAction::GenerateMollerBorn(G4Event* anEvent, const MollerVertex& v)
{
  G4cout << "[MolPol PGA] RCtype=0 → Born (no internal radiative corrections)" << G4endl;
  GenerateMollerStructurePath(anEvent, v, /*applyStructureRC=*/false);
}

void MolPolPrimaryGeneratorAction::GenerateMollerAlexander(G4Event* anEvent, const MollerVertex& v)
{
  G4cout << "[MolPol PGA] RCtype=1 → Alexander / Peking structure-function RC" << G4endl;
  GenerateMollerStructurePath(anEvent, v, /*applyStructureRC=*/true);
}

void MolPolPrimaryGeneratorAction::GenerateMollerStructurePath(
    G4Event* anEvent, const MollerVertex& v, G4bool applyStructureRC)
{
G4double pBeam = sqrt(v.beamE * v.beamE - electron_mass_c2 * electron_mass_c2);

      //Internal initial state radiation
      // LEGACY structure-function RC — gated by applyStructureRC (true iff RCtype==1).
      G4double s0 = 2 * electron_mass_c2 * pBeam * fLEcorFac;
      //~~The correct scale for the bremsstrahlung is the minimum of T and U
      G4double TUmin = 0.5 * s0 * ( 1 - fabs(cos(v.thcom)) );
      ...
      G4double p1 = pBeam/2 * x1 * ( 1 + cos(v.thcom) );
      G4double p2 = pBeam/2 * x1 * ( 1 - cos(v.thcom) );
      ...
      G4double cos2t = pow(cos(v.thcom),2);
      ...
      G4double weight = 1. * v.zLum * dPhaseSpace * sigma * strFct;
      ...
      G4double tX = v.direction.getX();
      G4double tY = v.direction.getY();

      //CALCULATE EACH ELECTRONS MOMENTUM DIRECTION COMPONENTS
      //electron #1
      G4double tX1 = tX + theta1 * cos(v.phcom);
      G4double tY1 = tY + theta1 * sin(v.phcom);
      ...
      //electron #2
      G4double tX2 = tX + theta2 * cos(v.phcom + pi);
      G4double tY2 = tY + theta2 * sin(v.phcom + pi);
      ...
      G4double Azz = -((7+pow(cos(v.thcom),2))*pow(sin(v.thcom),2))/pow(3+cos(v.thcom)*cos(v.thcom),2);

      fDefaultEvent->SetEffCrossSection(eff_sigma);
      fDefaultEvent->SetAsymmetry(Azz);
      fDefaultEvent->SetThCoM(v.thcom);
      fDefaultEvent->SetPhCoM(v.phcom);

      //GENERATE PARTICLES
      //electron #1
      G4double vtx_z = v.zpos + fZ;
      G4double KE1 = electron_mass_c2 * sqrt( 1 + pow(p1/electron_mass_c2,2)) - electron_mass_c2;
      particleGun->SetParticleEnergy( KE1 );
      particleGun->SetParticlePosition( G4ThreeVector(v.xpos, v.ypos, vtx_z) );
      particleGun->SetParticleMomentumDirection( G4ThreeVector( tX1, tY1, tZ1 ).unit() );
      fDefaultEvent->ProduceNewParticle(G4ThreeVector(v.xpos, v.ypos, vtx_z),
...
void MolPolPrimaryGeneratorAction::GenerateMollerMeradgen(
    G4Event* anEvent, const MollerVertex& v)
{
  G4cout << "[MolPol PGA] RCtype=2 → MERADGEN Approach A "
         << "(sample P=0, freeze pair, reweight ±)" << G4endl;
  const G4double beamE = v.beamE;
  const G4double thcom = v.thcom;
  const G4double phcom = v.phcom;
  const G4double xpos = v.xpos;
  const G4double ypos = v.ypos;
  const G4double zpos = v.zpos;
  const G4ThreeVector direction = v.direction;
  const G4double zLum = v.zLum;

  // Approach A: sample at pl=0, freeze lab e+/e- pair, weight_at ±1.
  const double elab_GeV = beamE / GeV;
  fMeradgen->InitBeam(elab_GeV);

  double rand4[4];
  bool ok = false;
  const int kMaxTries = 64;
  for (int attempt = 0; attempt < kMaxTries && !ok; ++attempt) {
    for (int i = 0; i < 4; ++i)
      rand4[i] = G4UniformRand();
    ok = fMeradgen->Generate(thcom, phcom, rand4);
  }
  if (!ok) {
    G4Exception("MolPolPrimaryGeneratorAction::GenerateMollerMeradgen",
                "MeradgenFiniteFail", JustWarning,
                "MERADGEN returned non-finite kinematics/weights after retries; skipping event.");
    return;
  }

  const MolPolMeradgen::LabParticles& lab = fMeradgen->Lab();
  const meradgen::MolPolEvent& kin = fMeradgen->Kinematics();
  const MolPolMeradgen::HelicityWeights& w = fMeradgen->Weights();

  G4double dPhaseSpace = fabs(cos(fthetaComMax) - cos(fthetaComMin));
  G4double scale = zLum * dPhaseSpace;

  // Map MERADGEN Approach A weights onto MolPol helicity ntuple columns.
  fDefaultEvent->fUnpolWght = scale * w.w0;
  fDefaultEvent->fpolPlusWghtZ  = scale * w.w_plus;
  fDefaultEvent->fpolMinusWghtZ = scale * w.w_minus;
  fDefaultEvent->fpolPlusWghtX  = 0.0;
  fDefaultEvent->fpolMinusWghtX = 0.0;
  fDefaultEvent->fpolPlusWghtY  = 0.0;
  fDefaultEvent->fpolMinusWghtY = 0.0;

  fDefaultEvent->fMeradUsed = true;
  fDefaultEvent->fMeradWeightsReady = fMeradgen->WeightsReady();
  fDefaultEvent->fMeradW0 = w.w0;
  fDefaultEvent->fMeradWplus = w.w_plus;
  fDefaultEvent->fMeradWminus = w.w_minus;
  fDefaultEvent->fMeradIch = kin.ich;
  fDefaultEvent->fMeradV = kin.vgen;
  fDefaultEvent->fMeradT1 = kin.t1gen;
  fDefaultEvent->fMeradZ = kin.zgen;
  fDefaultEvent->fMeradE1 = lab.scattered[3];
  fDefaultEvent->fMeradE2 = lab.recoil[3];
  for (int i = 0; i < 4; ++i)
    fDefaultEvent->fMeradRand4[i] = rand4[i];

  G4double Azz = 0.0;
  if (fMeradgen->WeightsReady()) {
    const G4double denom = w.w_plus + w.w_minus;
    if (denom > 0.0)
      Azz = (w.w_plus - w.w_minus) / denom;
  }
  fDefaultEvent->SetEffCrossSection(std::fabs(kin.xs0) > 0.0 ? std::fabs(kin.xs0) * microbarn
                                                            : 1.0 * microbarn);
  fDefaultEvent->SetAsymmetry(Azz);
  fDefaultEvent->SetThCoM(thcom);
  fDefaultEvent->SetPhCoM(phcom);

  // Lab momenta from MERADGEN (GeV) → Geant4 (MeV); rotate into beam direction.
  G4ThreeVector mom1(lab.scattered[0] * GeV, lab.scattered[1] * GeV, lab.scattered[2] * GeV);
  G4ThreeVector mom2(lab.recoil[0] * GeV, lab.recoil[1] * GeV, lab.recoil[2] * GeV);
  G4ThreeVector z0(0., 0., 1.);
  G4double ang = z0.angle(direction);
  if (ang > 1e-12) {
    G4ThreeVector axis = z0.cross(direction);
    if (axis.mag2() > 0.0) {
      mom1.rotate(ang, axis.unit());
      mom2.rotate(ang, axis.unit());
    }
  }

  G4double vtx_z = zpos + fZ;
  G4ThreeVector vtx(xpos, ypos, vtx_z);

  G4double E1 = lab.scattered[3] * GeV;
  G4double E2 = lab.recoil[3] * GeV;
  G4double KE1 = E1 - electron_mass_c2;
  G4double KE2 = E2 - electron_mass_c2;
  if (KE1 < 0.0) KE1 = 0.0;
  if (KE2 < 0.0) KE2 = 0.0;

  // ONE coincidence electron pair; photon is not tracked.
  particleGun->SetParticleEnergy(KE1);
  particleGun->SetParticlePosition(vtx);
  particleGun->SetParticleMomentumDirection(mom1.unit());
  fDefaultEvent->ProduceNewParticle(vtx, mom1, particleGun->GetParticleDefinition()->GetParticleName());
  particleGun->GeneratePrimaryVertex(anEvent);

  particleGun->SetParticleEnergy(KE2);
  particleGun->SetParticlePosition(vtx);
  particleGun->SetParticleMomentumDirection(mom2.unit());
  fDefaultEvent->ProduceNewParticle(vtx, mom2, particleGun->GetParticleDefinition()->GetParticleName());
  particleGun->GeneratePrimaryVertex(anEvent);
