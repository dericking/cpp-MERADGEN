...
G4bool MolPolEvent::EventIsSane(){
  ...
  if( std::isnan(fpolPlusWghtZ) || std::isinf(fpolPlusWghtZ) ) return false;
  if( std::isnan(fpolMinusWghtX) || std::isinf(fpolMinusWghtX) ) return false;
  if( std::isnan(fpolMinusWghtY) || std::isinf(fpolMinusWghtY) ) return false;
  if( std::isnan(fpolMinusWghtZ) || std::isinf(fpolMinusWghtZ) ) return false;
