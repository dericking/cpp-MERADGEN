...
MolPolMessenger::MolPolMessenger(){
    ...
    genSelectCmd->SetGuidance("Select generator: moller | beam | LUND");
    ...
    fRadCorrCmd->SetGuidance("Legacy bool for Alexander RC. Prefer /MolPol/RCtype.");
    fRadCorrCmd->SetGuidance("true → RCtype=1; false → RCtype=0 (does not clear RCtype=2).");
    fRadCorrCmd->SetParameterName("radCorrections",false);

    fRCtypeCmd = new G4UIcmdWithAnInteger("/MolPol/RCtype",this);
    fRCtypeCmd->SetGuidance("Radiative correction model for moller gen:");
    fRCtypeCmd->SetGuidance("  0 = none");
    fRCtypeCmd->SetGuidance("  1 = Alexander / Peking structure-function approximation");
    fRCtypeCmd->SetGuidance("  2 = MERADGEN Approach A (sample P=0, freeze pair, reweight)");
    fRCtypeCmd->SetParameterName("RCtype",false);
    fRCtypeCmd->SetRange("RCtype>=0 && RCtype<=2");
...
void MolPolMessenger::SetNewValue(G4UIcommand* cmd, G4String newValue){
  ...
  if( cmd == fRadCorrCmd ){
    ...
    // Legacy mapping: true→Alexander(1), false→none(0). Leave RCtype=2 alone.
    if( flag ){
      fPriGen->SetRCtype(1);
    } else if( fPriGen->GetRCtype() == 1 ){
      fPriGen->SetRCtype(0);
    } else if( fPriGen->GetRCtype() == 2 ){
      G4cout << "WARNING: /MolPol/radCorrections false ignored while RCtype=2 (MERADGEN). "
             << "Use /MolPol/RCtype 0 to disable." << G4endl;
    }
  }
  if( cmd == fRCtypeCmd ){
    fPriGen->SetRCtype(fRCtypeCmd->GetNewIntValue(newValue));
  }
