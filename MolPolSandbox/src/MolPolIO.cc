...
void MolPolIO::InitializeTree(){
  ...
  fTree->Branch("evUnpolWght",  &fUnpolWght,     "ev.unpolWght/D");
  fTree->Branch("evPolPlusWghtX", &fpolPlusWghtX, "ev.polPlusWghtX/D");
  fTree->Branch("evPolPlusWghtY", &fpolPlusWghtY, "ev.polPlusWghtY/D");
  fTree->Branch("evPolPlusWghtZ", &fpolPlusWghtZ, "ev.polPlusWghtZ/D");
  fTree->Branch("evPolMinusWghtX", &fpolMinusWghtX, "ev.polMinusWghtX/D");
  fTree->Branch("evPolMinusWghtY", &fpolMinusWghtY, "ev.polMinusWghtY/D");
  fTree->Branch("evPolMinusWghtZ", &fpolMinusWghtZ, "ev.polMinusWghtZ/D");
  ...
  // MERADGEN Approach A (shared kinematics + helicity weights)
  fTree->Branch("evMeradUsed",         &fMeradUsed,         "ev.meradUsed/I");
  fTree->Branch("evMeradWeightsReady", &fMeradWeightsReady, "ev.meradWeightsReady/I");
  fTree->Branch("evMeradW0",           &fMeradW0,           "ev.meradW0/D");
  fTree->Branch("evMeradWplus",        &fMeradWplus,        "ev.meradWplus/D");
  fTree->Branch("evMeradWminus",       &fMeradWminus,       "ev.meradWminus/D");
  fTree->Branch("evMeradIch",          &fMeradIch,          "ev.meradIch/I");
  fTree->Branch("evMeradV",            &fMeradV,            "ev.meradV/D");
  fTree->Branch("evMeradT1",           &fMeradT1,           "ev.meradT1/D");
  fTree->Branch("evMeradZ",            &fMeradZ,            "ev.meradZ/D");
  fTree->Branch("evMeradE1",           &fMeradE1,           "ev.meradE1/D");
  fTree->Branch("evMeradE2",           &fMeradE2,           "ev.meradE2/D");
  fTree->Branch("evMeradRand4",        fMeradRand4,         "ev.meradRand4[4]/D");
...
void MolPolIO::SetEventData(MolPolEvent *ev){
  ...
  fpolPlusWghtY = ev->fpolPlusWghtY;
  fpolPlusWghtZ = ev->fpolPlusWghtZ;
  fpolMinusWghtX = ev->fpolMinusWghtX;
  fpolMinusWghtY = ev->fpolMinusWghtY;
  fpolMinusWghtZ = ev->fpolMinusWghtZ;

  fMeradUsed = ev->fMeradUsed ? 1 : 0;
  fMeradWeightsReady = ev->fMeradWeightsReady ? 1 : 0;
  fMeradW0 = ev->fMeradW0;
  fMeradWplus = ev->fMeradWplus;
  fMeradWminus = ev->fMeradWminus;
  fMeradIch = ev->fMeradIch;
  fMeradV = ev->fMeradV;
  fMeradT1 = ev->fMeradT1;
  fMeradZ = ev->fMeradZ;
  fMeradE1 = ev->fMeradE1;
  fMeradE2 = ev->fMeradE2;
  for (int i = 0; i < 4; ++i)
    fMeradRand4[i] = ev->fMeradRand4[i];
  return;
}
