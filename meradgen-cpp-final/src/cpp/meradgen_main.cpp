...
namespace meradgen {
  ...
  void merad_init(double elab) {
    // Production: callers pass a full double elab. The parity tree's drivers
    // still float-round (FORTRAN merad_init is REAL*4).
    En = elab;
    s = 2.0 * (En * m + m2);
    als = s * (s - 4.0 * m2);
  ...
  void vectrec(const double vpgen[4], double r4) {
    t = vpgen[3] * vpgen[3] - vpgen[0] * vpgen[0] - vpgen[1] * vpgen[1] - vpgen[2] * vpgen[2];
    
    // Production: double atan2. FORTRAN REAL*4 atan2 stays in meradgen-cpp/.
    ...
    vprad[0] = -(sls * sl1 * sl8 * sp + (4.0 * al3 * al4 - s * al2 * al7) * cp) / denom;
    vprad[1] =  (sls * sl1 * sl8 * cp - (4.0 * al3 * al4 - s * al2 * al7) * sp) / denom;
    vprad[2] =  (als * al1 - s * (al7 + al2 * al4)) / (2.0 * std::sqrt(s) * al1 * sls);
    vprad[3] = -zgen / 2.0 / std::sqrt(s);
    phirad[0] = (sls * sl1 * sl8 * sp + (4.0 * al3 * al6 - s * al2 * al7) * cp) / denom;
    phirad[1] = (-sls * sl1 * sl8 * cp + (4.0 * al3 * al6 - s * al2 * al7) * sp) / denom;
    phirad[2] = std::sqrt(s) * (al7 + al2 * al6) / (2.0 * al1 * sls);
    phirad[3] = (vgen + zgen) / 2.0 / std::sqrt(s);
  }

  void meradgen(double ppl, const double vpgen[4], const double rand4[4]) {
    pl = ppl;
    t = vpgen[3] * vpgen[3] - vpgen[0] * vpgen[0] - vpgen[1] * vpgen[1] - vpgen[2] * vpgen[2];
    ...
    sirad_out = sirad;
    sinonr_out = sinonr_save;
#ifdef MERADGEN_PARITY_TRACE
    ptrc_d("sirad", sirad);
    ptrc_d("sitot", sitot);
#endif
    if (itest == 0) weight = sitot / xs0_save;
