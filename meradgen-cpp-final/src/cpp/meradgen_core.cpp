...
namespace meradgen {
  ...
  double vacpol(double tt) {
    // PDG lepton mass^2 via globals.cpp (m2, mu2, tau2). FORTRAN DATA stays in meradgen-cpp/.
    const double am2[3] = { m2, mu2, tau2 };                    // [source PDG 2024/25]
    ...
    for (int i = 0; i < 3; i++) {
      ...
      suml += 2.0 * (tt + a2) * allmi / 3.0
           - 10.0 / 9.0
           + 4.0 * a2 * (1.0 - a2 * allmi) / 3.0 / tt;
    }
    double aaa, bbb, ccc;
    if (tt < 1.0) {
      aaa = -1.345e-9;
