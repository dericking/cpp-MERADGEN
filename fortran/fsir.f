...
c The cross section of real photon emission
c t,t1,v,z are kinematic invariant
c pl is degree of polarization
c nn number of point where cross section is negative
c ikey=0 radiative cross section ds/dt/dv/dt1/dz
c ikey=1 radiative cross section integrated over z ds/dt/dv/dt1
c ikey=2 radiative cross section integrated over z and t1 ds/dt/dv
c ikey=-1 finite  part of
c radiative cross section integrated over z and t1 ds/dt/dv
	 implicit none
...
c	   sd=1d0/dsqrt(az*(zmax-z)*(z-zmin))
	 aj1=sd
	 aj2=z*sd
	 aj3=z**2*sd
	 aj4=zz*sd
...
c	print *,'fsir',fsir
	...
	fir=-4d0*((aj5+aj7+m2*aj1*vv**2+aj14)+(t-2d0*m2)*
     ...
     .	 (aj36+vv*aj4)-(s+t-2d0*m2)*(vv*aj6+aj24))
       fsir=fsir-alfa/4d0/pi**2*fir*sig(t,pl,0)
	endif
	end
